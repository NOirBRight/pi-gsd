...
```bash
_GSD_SHIM_NAME="gsd-tools.cjs"; GSD_TOOLS="$(node -e "console.log(require.resolve('@opengsd/gsd-core/get-shit-done/bin/gsd-tools.cjs'))" 2>/dev/null)"; if [ -n "$GSD_TOOLS" ] && [ -f "$GSD_TOOLS" ]; then gsd_run() { node "$GSD_TOOLS" "$@"; }; else _GSD_SHIM_NAME="gsd-tools.cjs"; _GSD_RUNTIME_ROOT="${RUNTIME_DIR:-$(git rev-parse --show-toplevel 2>/dev/null || pwd)}"; GSD_TOOLS="${_GSD_RUNTIME_ROOT}/get-shit-done/bin/${_GSD_SHIM_NAME}"; if [ -f "$GSD_TOOLS" ]; then gsd_run() { node "$GSD_TOOLS" "$@"; }; elif [ -f "${_GSD_RUNTIME_ROOT}/.claude/get-shit-done/bin/${_GSD_SHIM_NAME}" ]; then GSD_TOOLS="${_GSD_RUNTIME_ROOT}/.claude/get-shit-done/bin/${_GSD_SHIM_NAME}"; gsd_run() { node "$GSD_TOOLS" "$@"; }; elif command -v gsd-tools >/dev/null 2>&1; then GSD_TOOLS="$(command -v gsd-tools)"; gsd_run() { "$GSD_TOOLS" "$@"; }; elif [ -f "$HOME/.claude/get-shit-done/bin/${_GSD_SHIM_NAME}" ]; then GSD_TOOLS="$HOME/.claude/get-shit-done/bin/${_GSD_SHIM_NAME}"; gsd_run() { node "$GSD_TOOLS" "$@"; }; else echo "ERROR: gsd-tools.cjs not found at $GSD_TOOLS and gsd-tools is not on PATH. Run: npx -y @opengsd/gsd-core@latest --claude --local" >&2; exit 1; fi; fi
...
Handle responses:
- "Verify work": `If PI_GSD_NATIVE_CHAIN_OWNER is set, return control to the native orchestrator for /gsd-verify-work; otherwise Invoke /gsd-verify-work in Pi.`  then loop to dashboard.
- "Complete milestone": `If PI_GSD_NATIVE_CHAIN_OWNER is set, return control to the native orchestrator for /gsd-complete-milestone; otherwise Invoke /gsd-complete-milestone in Pi.` then exit.
...
```
If PI_GSD_NATIVE_CHAIN_OWNER is set, return control to the native orchestrator for /gsd-discuss-phase {PHASE_NUM} {manager_flags.discuss}; otherwise Invoke /gsd-discuss-phase {PHASE_NUM} {manager_flags.discuss} in Pi.
```
...
If PI_GSD_NATIVE_CHAIN_OWNER is set, return control to the native orchestrator for /gsd-plan-phase {N} --auto {manager_flags.plan}; otherwise Invoke /gsd-plan-phase {N} --auto {manager_flags.plan} in Pi.
...
If PI_GSD_NATIVE_CHAIN_OWNER is set, return control to the native orchestrator for /gsd-execute-phase {N} {manager_flags.execute}; otherwise Invoke /gsd-execute-phase {N} {manager_flags.execute} in Pi.
...
- Display the error clearly, then offer to fix it:
  ...
  - "Add permission and retry": Use `If PI_GSD_NATIVE_CHAIN_OWNER is set, return control to the native orchestrator for /update-config; otherwise Invoke /update-config in Pi.` to add the permission to `settings.local.json`, then re-spawn the background agent. Loop to dashboard.
  - "Run this phase inline instead": Dispatch the same action inline via the appropriate Skill — use `If PI_GSD_NATIVE_CHAIN_OWNER is set, return control to the native orchestrator for /gsd-plan-phase {N}; otherwise Invoke /gsd-plan-phase {N} in Pi.` if the failed action was planning, or `If PI_GSD_NATIVE_CHAIN_OWNER is set, return control to the native orchestrator for /gsd-execute-phase {N}; otherwise Invoke /gsd-execute-phase {N} in Pi.` if the failed action was execution. Loop to dashboard after.
...
- Display the error, then offer options via AskUserQuestion:
  ...
  - "Run inline instead": Dispatch the action inline via the appropriate Skill — use `If PI_GSD_NATIVE_CHAIN_OWNER is set, return control to the native orchestrator for /gsd-plan-phase {N}; otherwise Invoke /gsd-plan-phase {N} in Pi.` if the failed action was planning, or `If PI_GSD_NATIVE_CHAIN_OWNER is set, return control to the native orchestrator for /gsd-execute-phase {N}; otherwise Invoke /gsd-execute-phase {N} in Pi.` if the failed action was execution. Loop to dashboard after.
  - "Skip and continue": Loop to dashboard (phase stays in current state).
