...
```
Invoke /gsd-discuss-phase ${PHASE_NUM} in Pi
```
...
```
Invoke /gsd-ui-phase ${PHASE_NUM} in Pi
```
...
Agent(
  ...
  prompt="Run plan-phase for phase ${PHASE_NUM}: Invoke /gsd-plan-phase ${PHASE_NUM} in Pi"
)
```
...
```
Invoke /gsd-plan-phase ${PHASE_NUM} in Pi
```
...
Agent(
  ...
  run_in_background=true,
  prompt="Run execute-phase for phase ${PHASE_NUM}: Invoke /gsd-execute-phase ${PHASE_NUM} --no-transition in Pi"
)
```
...
```
Invoke /gsd-execute-phase ${PHASE_NUM} --no-transition in Pi
```
...
```
Invoke /gsd-plan-phase ${PHASE_NUM} --gaps in Pi
```
...
```
Invoke /gsd-ui-review ${PHASE_NUM} in Pi
```
...
Read and execute: `generated/workflows/references/autonomous-smart-discuss.md`
...
```
Invoke /gsd-audit-milestone in Pi
```
...
```
Invoke /gsd-complete-milestone ${milestone_version} in Pi
```
...
- [ ] Audit invoked via Invoke /gsd-audit-milestone in Pi
