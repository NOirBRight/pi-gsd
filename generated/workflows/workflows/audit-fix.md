...
```
Use the Pi subagent tool: subagent({agent: "gsd-executor", task: "Fix finding {ID}: {description}. Files: {file_refs}. Make the minimal change to resolve this specific finding. Do not refactor surrounding code."}). Wait for the subagent result before continuing this workflow.
```
